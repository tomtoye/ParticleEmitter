...
//we need position we want it, the colour of quad, size of it and where the camera is so it can face it
void Gizmos::billboard(glm::vec3& a_position, glm::vec4& a_colour, float& a_size, glm::mat4& a_camera)
{
	glm::vec3 vert[4]; //array to hold the position of each vert in quad

	glm::vec3 X(a_size, 0, 0); //x
	glm::vec3 Y(0, a_size*2, 0); //y (double x for explained reasons)

	glm::vec4 vX = glm::vec4(X, 0);
	vX = a_camera* vX; //face cam
	
	//verts for quad
	vert[0] = a_position - vX.xyz;
	vert[1] = a_position - vX.xyz + Y;
	vert[2] = a_position + vX.xyz + Y;
	vert[3] = a_position + vX.xyz;


	//drawing each tri for quad

	addTri(vert[0], vert[3], vert[2], a_colour);
	addTri(vert[0], vert[2], vert[1], a_colour);
